// File: Executables/calculate_orbital_moment.cpp
#include <algorithm>
#include <filesystem>
#include <fstream>
#include <iomanip>
...
static core::GridData make_kmesh(
    ...
    rgmpi::abort_all(
        "calculate_orbital_moment: unsupported kmesh.type = "
        + mesh.type
    );
...
static core::PathData make_kpath(
    const rg::RG_Structure& st,
    const config::CalOrbitalMomentConfig& cfg
) {
    ...
    rgmpi::abort_all(
        "calculate_orbital_moment: unsupported kpath.type = "
        + cfg.kpath_type
    );
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        const auto cfg =
            config::read_cal_orbital_moment_config(config_file);
        ...
        if (cfg.model == "sk") {
            ...
            rgmpi::abort_all(
                "calculate_orbital_moment: model must be sk or kp"
            );
        }
        ...
        for (int b : bands) {
            if (b < 0 || b >= dim) {
                rgmpi::abort_all(
                    "calculate_orbital_moment: band index out of range"
                );
            }
        }
        ...
        const std::string run_stamp =
            rgio::make_time_stamp("orbital_moment");

        if (rank == 0) {
            const fs::path out_root =
                base_dir
                / (
                    std::string("orbital_moment_")
                  + cfg.model
                  + "_"
                  + suffix
                )
            ...
            std::cout << "=== calculate_orbital_moment ===\n";
        ...
        try {
            rgmpi::abort_all(
                std::string("calculate_orbital_moment FAILED: ")
                + e.what(),
                1
            );
        } catch (...) {}
