function H = hysteresis_realchi_T_SinglePolar()
% plot_hysteresis_realchi_T
...
%
% Hysteresis logic (robust, artificial-style):
%   For each dop, find ALL minima of full 2D F(psi1,psi2) using:
%     dense scan -> discrete local minima seeds -> fminsearch repeats
%     -> cluster -> Hessian positive definite filter
%   Then do continuation:
%     forward: pick minimum closest to previous solution
%     backward: same, sweeping back
...
%     default_root = "/Users/haoranyan/Library/CloudStorage/OneDrive-Emory/Rhombohedral_SC/rhombohedral_project/data/chi_sk_mu_200/D0.067";
    ...
    for i = 1:N
        ...
        if isempty(mins)
            % fallback: just minimize from prev
    ...
    prev = psi_f(end,:); % start from forward end
    ...
    title(ax1, sprintf('$T=%.6g$, q=(%d,%d)', T0, iq_pick, jq_pick), ...
    ...
    % ============================================================
    %   psi-X hysteresis trajectory plot  (ABS + fixed colors)
    % ============================================================
    ...
    % ----- forward path (BLUE solid) -----
    plot(ax, psi_f_abs(:,1), psi_f_abs(:,2), ...
        '-', ...
        'Color',[0 0.4470 0.7410], ...   % MATLAB default blue
        'LineWidth',2);

    % ----- backward path (ORANGE-RED dashed) -----
    plot(ax, psi_b_abs(:,1), psi_b_abs(:,2), ...
        '--', ...
        'Color',[0.85 0.33 0.10], ...    % orange-red
        'LineWidth',2);

    % ----- mark start & end (optional but clean) -----
    plot(ax, psi_f_abs(1,1), psi_f_abs(1,2), ...
        'o', 'Color',[0 0.4470 0.7410], ...
        'MarkerSize',8,'LineWidth',2);

    plot(ax, psi_f_abs(end,1), psi_f_abs(end,2), ...
        'o', 'Color',[0.85 0.33 0.10], ...
        'MarkerSize',8,'LineWidth',2);

    xlabel(ax,'$|\psi|$','Interpreter','latex');
    ylabel(ax,'$|X|$','Interpreter','latex');

    title(ax, sprintf('Hysteresis trajectory at $T=%.6g$',T0), ...
        'Interpreter','latex','FontWeight','normal','FontSize',fs);

    legend(ax, {'forward','backward'}, ...
    ...
    out_png2 = fullfile(out_dir, sprintf( ...
        "hyst_realchi_trajectory_T%.6g_iq%d_jq%d_lambda%.3g.png", ...
        T0, iq_pick, jq_pick, par.lambda));
    ...
    out_png = fullfile(out_dir, sprintf("hyst_realchi_dense_T%.6g_iq%d_jq%d_lambda%.3g.png", ...
        T0, iq_pick, jq_pick, par.lambda));
...
function D = load_chi_grid_newpatch_absq_recursive_(root_dir, iq_pick, jq_pick)
    ...
    for k = 1:numel(L)
        ...
        % out-of-range quick reject (your files usually have small ranges like [-133,-132])
    ...
    % use ismember (avoid float == issues)
    for i = 1:numel(Tvals)
...
function H = parse_header_newpatch_(fpath)
    ...
    for t = 1:4000   % header can be long (RG_Structure block)
        ln = fgetl(fid);
    ...
    % iq/jq range is optional; T and dop are required
    H.ok = isfinite(H.T_K) && isfinite(H.dop);
end

% =====================================================================
% Robust column detection (works even if only a few rows exist)
% Supports:
...
function C = detect_cols_(M)
    ...
    if ncol >= 7
        ...
        if intlike(col1) && (min(col1)==0 || min(col1)==1) && intlike(col2) && intlike(col3)
            % iq/jq typically can be negative or have magnitude > 5
