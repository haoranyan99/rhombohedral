...
% IMPORTANT: coefficients a are computed from SHIFT-AVERAGED chi:
%   chi_art(mu) = 0.5*(chi(mu+dmu) + chi(mu-dmu)),
%   dmu = 0.5 * artificial_polar * 1e-3 (eV).
%
% This version:
%   - plots ONLY 3 separate figures:
%       1) a1(doping)
%       2) |psi|(doping)
%       3) |X|(doping)
%   - does NOT auto save
%   - xlim uses ONLY the actual scan range
...
% =========================================================
% 7) plot (THREE SEPARATE FIGURES, NO AUTO SAVE)
% =========================================================
...
% Fig 1: a1(doping)
% -------------------------
fig1 = figure('Color','w','Units','pixels','Position',[80 80 720 560], ...
    'Name','a1 vs doping');

ax1 = axes(fig1);
plot(ax1, dop_fwd, a1_f, 'k-','LineWidth',2); hold(ax1,'on');
plot(ax1, dop_bwd, a1_b, 'k--','LineWidth',2); hold(ax1,'off');
grid(ax1,'on'); box(ax1,'on');
xlim(ax1, [xlo xhi]);
yline(ax1, 0, '--', 'LineWidth', 1.2);

xlabel(ax1,'doping (10^{12} cm^{-2})','Interpreter','tex');
ylabel(ax1,'$a_1$','Interpreter','latex');
title(ax1, ['${a_1}$, ' tShort], ...
...
% Fig 2: |psi|(doping)
% -------------------------
fig2 = figure('Color','w','Units','pixels','Position',[120 120 720 560], ...
    'Name','psi hysteresis');

ax2 = axes(fig2);
plot(ax2, dop_fwd, psi_f_abs_plot(:,1), '-',  'LineWidth',2); hold(ax2,'on');
plot(ax2, dop_bwd, psi_b_abs_plot(:,1), '--', 'LineWidth',2); hold(ax2,'off');
grid(ax2,'on'); box(ax2,'on');
xlim(ax2, [xlo xhi]);

xlabel(ax2,'doping (10^{12} cm^{-2})','Interpreter','tex');
ylabel(ax2,'$|\psi|$','Interpreter','latex');
title(ax2, ['$|\psi|$ hysteresis, ' tShort], ...
    'Interpreter','latex','FontWeight','normal','FontSize',fs);
...
% Fig 3: |X|(doping)
% -------------------------
fig3 = figure('Color','w','Units','pixels','Position',[160 160 720 560], ...
    'Name','X hysteresis');

ax3 = axes(fig3);
plot(ax3, dop_fwd, psi_f_abs_plot(:,2), '-',  'LineWidth',2); hold(ax3,'on');
plot(ax3, dop_bwd, psi_b_abs_plot(:,2), '--', 'LineWidth',2); hold(ax3,'off');
grid(ax3,'on'); box(ax3,'on');
xlim(ax3, [xlo xhi]);

xlabel(ax3,'doping (10^{12} cm^{-2})','Interpreter','tex');
ylabel(ax3,'$|X|$','Interpreter','latex');
title(ax3, ['$|X|$ hysteresis, ' tShort], ...
    'Interpreter','latex','FontWeight','normal','FontSize',fs);
