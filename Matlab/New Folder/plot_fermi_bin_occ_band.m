...
[fname, fpath] = uigetfile(fullfile(default_dir, '*.bin'), ...
...
scatter(F.kx, F.ky, marker_size, plot_data, ...
...
cb = colorbar;
ylabel(cb, cb_label, 'FontSize', 16);

plot_BZ_boundary(L);

scatter(L.Gamma(1), L.Gamma(2), 80, 'c', 'filled');
text(L.Gamma(1), L.Gamma(2), '  \Gamma', ...
    'FontSize',16,'FontWeight','bold');

scatter(L.M(1), L.M(2), 80, 'c', 'filled');
text(L.M(1), L.M(2), '  M', ...
    'FontSize',16,'FontWeight','bold');

scatter(L.K(1), L.K(2), 80, 'c', 'filled');
text(L.K(1), L.K(2), '  K', ...
    'FontSize',16,'FontWeight','bold');

scatter(L.Kp(1), L.Kp(2), 80, 'c', 'filled');
text(L.Kp(1), L.Kp(2), '  K''', ...
    'FontSize',16,'FontWeight','bold');
...
title(sprintf('Band %d   EF = %.4f eV', band_idx, F.EF), ...
    'FontSize',18);
...
plot(corners(:,1), corners(:,2), 'c-', 'LineWidth', 2.5);
