function out_png_list = plot_dos_shiftMean_energyShift_vs_x()
% Plot shift-mean DOS vs x-axis = doping or mu
%
% Rule:
...
%
% x is parsed robustly from numeric columns first, then header fallback.
...
mu_shift_meV = 10;      % energy shift in meV
...
shift_eV = 1e-3 * mu_shift_meV;
fprintf("[energy shift] %.12g eV\n", shift_eV);
...
for k = 1:numel(file)
    ...
    % ==========================================
    % shift-mean on ENERGY axis first
    % then parse x from shifted positions
    % ==========================================
    x_plot = nan(size(E));
    y_plot = nan(size(E));
    ...
    if isempty(x_plot)
        fprintf("[skip] no valid shifted points: %s\n", in_path);
        continue;
    end
    ...
    % original curve for overlay (x from original parse, DOS original)
    ...
    fig = figure('Color','w','Units','pixels','Position',[120 120 820 560], ...
        'Name', sprintf('shift-mean DOS vs %s', x_mode));
    ...
    if overlay_original
        plot(ax, x_org, y_org, '-', 'LineWidth', 2.0, 'DisplayName', 'original DOS');
    end

    plot(ax, x_plot, y_plot, '--', 'LineWidth', 2.2, ...
        'DisplayName', sprintf('shift-mean DOS, \\DeltaE=%.4g meV', mu_shift_meV));
    ...
    title(ax, sprintf('%s | shift on E, plot vs %s', base, x_mode), ...
        'Interpreter','none', 'FontWeight','normal');

    legend(ax, 'Location', 'best');

    if save_png
        out_png = fullfile(path, base + "_shiftMean_Eshift_vs_" + x_mode + ".png");
