function db = build_chi_mat_database()
% BUILD_CHI_MAT_DATABASE
% UI version:
%   1) choose chi root folder by popup
%   2) parse root folder name, e.g. chi_sk_mu_400
%   3) recursively scan:
%        D{XXX}/polar_meV{XXX}/T{XXX}/mu{XXXX}/chi*.txt
...
% -> model = "sk", mode = "mu", Nk = 400
    ...
    root_folder = uigetdir(default_folder, ...
        'Select chi root folder to parse (e.g. chi_sk_mu_400)');
    ...
    meta_root = parse_root_folder_name(root_name);

    fprintf('Selected root folder:\n  %s\n', root_folder);
    fprintf('Parsed root metadata: model=%s, mode=%s, Nk=%g\n', ...
        meta_root.model, meta_root.mode, meta_root.Nk);

    % ---------------- recursively find chi*.txt ----------------
    files = dir(fullfile(root_folder, "**", "chi*.txt"));
    fprintf("Found %d chi*.txt files under:\n  %s\n", numel(files), root_folder);

    if isempty(files)
        warning('No chi*.txt files found.');
        db = [];
        return;
    end
    ...
    % ---------------- waitbar ----------------
    wb = waitbar(0, 'Building MAT database...', 'Name', 'build_chi_mat_database');
    ...
    for k = 1:numel(files)
        ...
        [Dval, polarVal, Tval, muVal] = parse_chi_tree_path(relpath);
    ...
    db.pattern      = "{root}/D{XXX}/polar_meV{XXX}/T{XXX}/mu{XXXX}/chi*.txt";
    ...
    db.index.D          = [recs.D]';
    db.index.polar_meV  = [recs.polar_meV]';
    db.index.T          = [recs.T]';
    db.index.mu         = [recs.mu]';
    ...
    db.table = table( ...
        ...
        string({recs.mode})', ...
        [recs.Nk]', ...
        'VariableNames', {'relpath','name','D','polar_meV','T','mu','model','mode','Nk'});

    % ---------------- choose SAVE folder AFTER processing ----------------
    save_folder = uigetdir(default_folder, ...
        'Select folder to save MAT database');
    ...
    fprintf("Saved MAT database:\n  %s\n", out_mat);

    msgbox(sprintf('MAT database saved successfully:\n%s', out_mat), ...
           'Success', 'help');
end

function meta = parse_root_folder_name(root_name)
    meta = struct();
    meta.prefix = "";
    meta.model  = "";
    meta.mode   = "";
    ...
    if numel(parts) >= 4
        ...
        if ~isempty(tok)
            ...
            warning('Root folder name "%s" does not match expected pattern chi_sk_mu_400.', root_name);
        end
    end
end
...
function [Dval, polarVal, Tval, muVal] = parse_chi_tree_path(relpath)
% Parse path like:
%   D0.067/polar_meV200/T4.000/mu1.005600/chi_xxx.txt

    relpath = char(relpath);

    Dval     = parse_one(relpath, '(?:^|[/\\])D([-+]?\d*\.?\d+(?:[eEdD][-+]?\d+)?)');
    polarVal = parse_one(relpath, '(?:^|[/\\])polar_meV([-+]?\d*\.?\d+(?:[eEdD][-+]?\d+)?)');
