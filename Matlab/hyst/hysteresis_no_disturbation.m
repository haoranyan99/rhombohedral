...
if use_parallel && nTask > 1
    ...
    for kk = 1:nTask
        ...
        fprintf("---- artificial polar = %.6g meV, dmu = %.6g eV ----\n", pmev, dmu);
...
for it = 1:nT
    for ip = 1:nP
        ...
        write_hyst_data_multiP(R, it, ip, nT, nP);
...
function write_hyst_data_multiP(R, it, ip, nT, nP)

% ============================================================
% Filename rule:
% nT = 1, nP = 1  -> hyst.mat
% nT > 1, nP = 1  -> hyst_allT.mat
% nT = 1, nP > 1  -> hyst_allPolar.mat
% nT > 1, nP > 1  -> hyst_allT_allPolar.mat
% ============================================================
...
if nP > 1
    suffix = suffix + "_allPolar";
end

fname = fullfile(pwd, "hyst" + suffix + ".mat");
